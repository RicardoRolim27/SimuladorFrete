package ceep.tcc.models;

import java.io.Serializable;

import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;
import javax.persistence.Id;
import javax.persistence.JoinColumn;
import javax.persistence.ManyToOne;
import javax.persistence.Table;

import com.fasterxml.jackson.annotation.JsonIgnore;

@Entity
@Table(name = "adicional_frete")
public class AdicionalFrete implements Serializable {
	private static final long serialVersionUID = 1L;

	@Id
	@GeneratedValue(strategy = GenerationType.IDENTITY)
	private Long id;
	private double pedagio;
	private double valorAdicional;
	private double valorDoGris;
	private double valorExcedente;

	@JsonIgnore
	@ManyToOne
	@JoinColumn(name = "transportadora_id")
	private Transportadora transportadoras;

	public AdicionalFrete() {
	}

	public AdicionalFrete(Long id, double pedagio, double valorAdicional, double valorDoGris, double valorExcedente,
		Transportadora transportadoras) {
		super();
		this.id = id;
		this.pedagio = pedagio;
		this.valorAdicional = valorAdicional;
		this.valorDoGris = valorDoGris;
		this.valorExcedente = valorExcedente;
		this.transportadoras = transportadoras;
	}

	public Long getId() {
		return id;
	}

	public void setId(Long id) {
		this.id = id;
	}

	public double getPedagio() {
		return pedagio;
	}

	public void setPedagio(double pedagio) {
		this.pedagio = pedagio;
	}

	public double getValorAdicional() {
		return valorAdicional;
	}

	public void setValorAdicional(double valorAdicional) {
		this.valorAdicional = valorAdicional;
	}

	public double getValorDoGris() {
		return valorDoGris;
	}

	public void setValorDoGris(double valorDoGris) {
		this.valorDoGris = valorDoGris;
	}

	public double getValorExcedente() {
		return valorExcedente;
	}

	public void setValorExcedente(double valorExcedente) {
		this.valorExcedente = valorExcedente;
	}
	

	public Transportadora getTransportadoras() {
		return transportadoras;
	}

	public void setTransportadoras(Transportadora transportadoras) {
		this.transportadoras = transportadoras;
	}

	@Override
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + ((id == null) ? 0 : id.hashCode());
		return result;
	}

	@Override
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (obj == null)
			return false;
		if (getClass() != obj.getClass())
			return false;
		AdicionalFrete other = (AdicionalFrete) obj;
		if (id == null) {
			if (other.id != null)
				return false;
		} else if (!id.equals(other.id))
			return false;
		return true;
	}
}
